...
# hermes_integrations is packages/rsg-hermes-core/hermes_integrations
# parents[0]=hermes_integrations, [1]=rsg-hermes-core, [2]=packages, [3]=repo
...
def _repo_csv(path: Path) -> Path:
    if path.is_file():
        return path
    # Fallback when installed as a package (no repo docs/).
...
def plan_fields(
    *,
    ...
    for row in rows or load_field_rows():
        ...
        if csv_type == "Lookup (Policies)" and "Policies" not in existing_modules:
            steps.append(
                {
                    "action": "skip",
                    "api_name": api_name,
                    "reason": "Policies module is not in this org yet",
                }
            )
            continue
        steps.append(
