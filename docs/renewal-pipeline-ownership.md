...
This file lives in **rsg-hermes** (the system is
...
- **Gen 2 — the CRM native (RsgCore module):** scheduled jobs + hooks. **Live.**
...
|---|---|---|
| Create Renewal record from an expiring policy | **the CRM (native)** | `SyncRenewalsFromPolicies` scheduled job (daily 6am) + `Policy/ActivateAutomation` hook → `RenewalOrchestrator::syncFromPolicy()` |
| Derive renewal fields (premium / LOB / carrier / expiration / urgency) | **the CRM (native)** | `Renewal/DeriveFields` hook → `RenewalOrchestrator` |
...
| Worksheet UI + required-field enforcement (Won can't be faked) | **the CRM** | `Renewal` entityDefs (4 checkbox bools) + `clientDefs` dynamic logic + detail layout |
| Task completion → file worksheet (Google Doc) + Slack card | **Hermes** | `hermes/renewals/complete.py` via `POST /renewals/complete` (fired by the CRM `Task/SendServiceWebhook` dispatcher) |
| Slack "Acknowledge" button (idempotent) | **Hermes** | `hermes/integrations/slack_socket.py` (`renewal_ack_*`) |
| Commission on won (renewal & new business) | **the CRM (native)** | `Renewal/CreateCommissionLedger` + `Opportunity/CreateCommissionLedger` hooks (idempotent upsert) |
...
- **Native bare-task creation** — `RenewalOrchestrator::createInitialTaskIfMissing`
  ...
  sweep. **Hermes owns task creation.** the CRM still owns Renewal-record creation.
- NowCerts↔the CRM sync in n8n — Hermes owns NowCerts sync.

## The rule
A renewal flows: **the CRM creates the record → Hermes creates the task →
the CRM enforces the worksheet → Hermes handles completion → the CRM books the
