...
This document is the **source-to-table contract** between n8n, Hermes, Supabase, and EspoCRM. Implementations live in **rsg-hermes** (`hermes/sync/pipeline.py`, `hermes/sync/bidirectional.py`); orchestration on Elestio uses **n8n** plus **Hermes** workers.

## Design principle

**Supabase is the hub.** No production path should write EspoCRM from n8n without also writing the same intent through Supabase flow tables (staging, queue, audit), except for emergencies.

---

## A. NowCerts / Momentum → Supabase → EspoCRM
...
7. **`outbound_sync_queue`** — One row per Espo write intent: must include **`run_id`**, **`mapping_id`** (nullable), **`attempt_count`** (start at `0`), `object_type`, `action`, `payload`, `status` = `queued`.  
...
8. **Hermes** — Worker drains `outbound_sync_queue` / `crm_write_queue` and applies writes to EspoCRM.
...
Today many workflows only: read `sync_state` → call Espo → write `sync_state`. To match this contract, insert nodes **before** Espo writes:
...
- Insert **`outbound_sync_queue`** rows (do not call EspoCRM directly for those records).
- After Hermes processes the queue (or on a schedule), read **`sync_audit_log`** / **`sync_errors`** for alerting.

Defer EspoCRM HTTP nodes until the queue path is stable, or keep them behind a feature flag.

---

## B. EspoCRM → Supabase (mirror)
...
2. Poll Espo **`Account`** / **`Policy`** (or webhooks → staging only) by `modifiedAt` &gt; cursor.
3. **`inbound_sync_staging`** — Raw Espo payloads for the run (`source_system` = `espocrm`).
...
| Area | Location |
|------|----------|
| NowCerts → Espo pipeline | `hermes/sync/pipeline.py` |
| Espo → Supabase mirror | `hermes/sync/bidirectional.py` |
