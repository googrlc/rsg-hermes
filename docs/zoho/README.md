...
| `fields_document_registry.csv` | Custom module **Document_Registry** ← Nextcloud file metadata |
...
1. Create custom modules **Policies**, **Renewal_Events**, **Renewals**, **AMS_Write_Queue**, **Document_Registry** (if not using Zoho Insurance vertical for Policies). Document_Registry: `python scripts/zoho_document_registry_setup.py` (dry-run) / `--apply` with `ZohoCRM.settings.modules.CREATE` + `ZohoCRM.settings.fields.CREATE`.
