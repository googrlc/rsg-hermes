...
function checkpointKeyForTitle(title) {
  return ALIAS_TO_KEY[String(title || "").trim().toLowerCase()] || null;
}

function statesFromTasks(tasks) {
  const out = {};
  (tasks || []).forEach((task) => {
...
function dumpCheckpointState(states) {
  const slim = {};
...
function mergeCheckpointStates(row, tasks) {
...
function remainingRequired(stage, states) {
  const stored = storedDeskStage({ Stage: stage });
...
function scorecard(stage, states) {
  const stored = storedDeskStage({ Stage: stage });
...
function completeCheckpoint(stage, states, key, opts) {
  const stored = storedDeskStage({ Stage: stage });
...
function checkpointsForStage(stage, states) {
  const stored = storedDeskStage({ Stage: stage });
  const st = states || {};
...
function nextRequiredAction(stage, states) {
  const stored = storedDeskStage({ Stage: stage });
