...
Hermes is the integration layer — this script only stands up the metadata
registry in Zoho. File bytes go to Nextcloud via
``hermes.documents.registry``.
...
def apply_plan(client: ZohoSettingsClient, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not plan["module_exists"]:
        ...
        # Refresh field list after module create (Name exists now).
...
def main() -> int:
    ...
    if not args.apply:
        ...
        print("Nextcloud_File_URL must be marked Required on the layout after create.")
        return 0
