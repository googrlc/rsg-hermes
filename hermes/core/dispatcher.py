...
class Dispatcher:
    ...
    def __init__(self, *, use_openai: bool = False) -> None:
        ...
        self._routes: list[tuple[re.Pattern[str], Handler | str]] = [
            ...
            (
                re.compile(r"\b(data\s+quality|dq\s+report|audit\s+crm|crm\s+audit|quality\s+check)\b", re.I),
                "data_quality",
            ),
            (
                re.compile(
            ...
            (
                re.compile(r"^\s*(met|talked|spoke|just\s+met|new\s+lead|log\s+lead|intake)\b", re.I),
                "intake",
            ),
        ]
    ...
    def _call_handler(self, handler: Handler | str, client: "EspoClient", text: str) -> DispatchResult:
        ...
        return handler(client, text)  # type: ignore[return-value]
