"""Case and task persistence on the shared agency CRM schema.

Cases live in ``agency_crm_cases`` with tasks in ``agency_crm_tasks``, documents
in ``agency_crm_document_links`` and an audit trail in
``agency_crm_case_events``. None of that is renewal-specific — the same tables
carry marketing, service and claims cases — but all of it used to live in
``hermes/renewals/cases.py``, whose own docstring conceded the tables served
"marketing/service/claims/etc." while filing the code under renewals.

That misfiling is why the cases app could not be extracted: creating a task went
through the renewal module, so cases depended on renewals for work that has
nothing to do with a renewal. The renewal-specific half — the renewal-event
identity, its idempotency key, its default task set — stays in
``hermes/renewals/cases.py`` and imports this.
"""
...
# The 1:1 renewal sidecar. Named here, against the grain of everything above,
# because `delete_case` has to clear it: a case's children must all go before
# the case does, and leaving this one behind orphans a row that points at a case
# that no longer exists. When renewals becomes its own repo this is the line to
# revisit — the child list wants to be extensible rather than to know the name
# of one app's table.
