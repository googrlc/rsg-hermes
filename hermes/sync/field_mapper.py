...
def map_nowcerts_policy_to_espo_commission(
    nc_policy: dict[str, Any],
    *,
    policy_id: str,
    account_id: str | None = None,
    ...
    """Transform a NowCerts Policy into an EspoCRM ``Commission`` payload.

    Mirrors the Policy push: whenever a policy lands in EspoCRM, a Commission
    record is written into the custom commission module and linked to that
    Policy (``policyId``) and its Account (``accountId``).

    Uses the documented Commission fields (``commissionRate``,
    ``estimatedCommission``, ``effectiveDate``, ``carrier``, ``name``). The
    caller conforms this payload against live Commission metadata before send,
    so any field the entity doesn't actually expose is dropped rather than
    failing the write. ``commissionType`` is intentionally omitted — it is an
    enum whose valid values aren't known here, and sending an invalid enum value
    would 400 (a metadata check can't validate enum *values*, only field names).
    """
    number = str(
        policy_number
        or nc_policy.get("number")
    ...
    payload: dict[str, Any] = {
        "name": number or (f"{carrier_name} commission" if carrier_name else "Commission"),
        "policyId": policy_id,
    }
    if account_id:
        payload["accountId"] = account_id
    ...
    rate = _first_non_none(
        nc_policy, "agencyCommissionPercent", "AgencyCommissionPercent", "commissionRate",
    )
    if rate is not None:
        try:
            payload["commissionRate"] = float(rate)
        except (TypeError, ValueError):
            pass

    amount = _first_non_none(
        nc_policy, "agencyCommissionValue", "AgencyCommissionValue", "commissionAmount",
    )
    if amount is not None:
        try:
            payload["estimatedCommission"] = float(amount)
        except (TypeError, ValueError):
            pass

    eff = _strip_date(nc_policy.get("effectiveDate") or nc_policy.get("EffectiveDate"))
    if eff:
        payload["effectiveDate"] = eff
