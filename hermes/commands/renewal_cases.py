...
def _ensure_case(resolved: resolve.ResolvedPolicy, supa: "SupabaseClient") -> tuple[dict[str, Any], bool] | None:
    ...
    return cases.create_case(
        ...
        assigned_to=_DEFAULT_ASSIGNEE,
        summary=f"Renewal review — {p.get('accountName') or p.get('policyNumber')}",
        created_by="hermes-nl",
    )
    ...
    *,
    ...
    if _WITH_TASKS_RE.search(text):
        tasks_made = cases.create_tasks(
            ...
            tasks=cases.default_tasks(assigned_to=case.get("assigned_to")),
            default_assignee=case.get("assigned_to"),
        )

    verb = "Opened" if created else "Found existing"
    msg = (
        f"🗂️ {verb} renewal case — {case.get('client_name') or p.get('policyNumber')} "
        f"(policy #{case.get('policy_number')})\n"
        f"- Case: `{str(case.get('id'))[:8]}` · status {case.get('status')} · assigned {case.get('assigned_to')}"
    )
    if tasks_made:
...
def create_tasks_handle(
    ...
    *,
    ...
    lines = [f"✅ Added {len(tasks_made)} renewal tasks for policy #{case.get('policy_number')} "
             f"(assigned {case.get('assigned_to')}):"]
